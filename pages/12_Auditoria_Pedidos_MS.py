...
for var in ["CONNEXA_PG_HOST", "CONNEXA_PG_PORT", "CONNEXA_PG_DB", "CMSP_HOST", "CMSP_PORT", "CMSP_DB"]:
...
ORDER BY 1 DESC
"""
# Consulta para leer pre‑carga desde diarco_data (Postgres)
SQL_PRECarga_PG = """
SELECT c_proveedor, c_articulo, c_sucu_empr, q_bultos_kilos_diarco, f_alta_sist,
       ...
       m_procesado, f_procesado, u_prefijo_oc, u_sufijo_oc, c_compra_connexa,
...
def main():
    st.title("Control Pedidos CONNEXA_MS → SGM")
    st.caption("Connexa → Pre‑carga Connexa → SGM (SQL Server)")
